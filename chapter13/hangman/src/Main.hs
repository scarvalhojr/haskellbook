...
loadWords = do
  ...
  let words = filter validLength (lines dict)
  putStrLn $ "Loaded " ++ (show (length words)) ++ " words."
...
randomWord (WordList words) = do
  rnd <- randomRIO (0, (length words) - 1)
  return $ words !! rnd
...
fillInCharacter p@(Puzzle word disc guess) c
  | elem c guess  = p
